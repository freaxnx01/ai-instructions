...
(Questions 1-12 resolved (10 on 2026-05-25, #11 and #12 on 2026-05-26).
Question #13 raised 2026-05-27, awaiting resolution.)

13. Snippet placement in regenerated CLAUDE.md files. Decision #12 says
    the implementer-role snippet goes "directly after Project Overview,
    before Essential Commands." That works for hand-written CLAUDE.md
    files (e.g. `bridge/CLAUDE.md`). It does NOT work for implementer
    repos whose CLAUDE.md is auto-generated by `/sync-ai-instructions`
    — currently `agent-pipeline` (uses the `ci` stack) and
    `ai-instructions` itself (uses the `dotnet-blazor` stack as its
    own template). Editing those directly would be wiped on the next
    regen.

    Options to evaluate:
    A. Per-project role addendum file (e.g. `.ai/role.md`) that the
       sync skill concatenates after the assembled base+stack content.
    B. Preservation markers in CLAUDE.md (sync skill skips content
       between `<!-- KEEP -->` markers on regen).
    C. Separate `WORKFLOW-ROLE.md` referenced from CLAUDE.md via a
       stable footer the sync skill always emits.
    D. Per-project role parameter to `/sync-ai-instructions` that pulls
       the snippet text in at regen time.

    Until resolved: do NOT add the implementer-role snippet to
    `agent-pipeline/CLAUDE.md` or `ai-instructions/CLAUDE.md`. Bridge
    is unaffected (hand-written CLAUDE.md).
