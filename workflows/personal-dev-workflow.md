...
RepoVisibilityRoleNotesai-instructionspublicImplementerAI tooling, CLAUDE.md/SKILL.md, multi-stack. Hosts this workflow doc. CLI alias aii.homelab vaultprivate (separate, Git-backed, not on GitHub)n/a (vault, not workflow infra)Homelab infra notes only — correctly scoped. Lives at /mnt/c/Users/freax/Documents/Obsidian/homelab/.agent-pipeline (renamed from claude-pipeline)publicImplementerGH Actions pipeline replicating Copilot Coding Agent. Hosts its own design doc. CLI alias agp.bridge (renamed from clrepo)publicImplementerPersonal dev cockpit: agent session launcher, dashboards for issues/PRs/Action runs. CLI alias brg.ideas-labprivateImplementerIdeas-before-projects incubator. CLI alias idl.ideas-lab (work mirror)private (separate work account)ImplementerWork/business-scoped mirror of ideas-lab under a separate private account. Same structure and graduation path; content never crosses into public repos. CLI alias widl.quicktask-vikunja etc.variesConsumerReal projects. Follow workflow as background guidance.
...
12. CLAUDE.md snippet placement (resolved 2026-05-26)
    ...
    Status: `bridge/CLAUDE.md` backfilled 2026-05-27. `agent-pipeline`
    follows Decision #13 instead (regenerated CLAUDE.md).

13. Snippet placement in regenerated CLAUDE.md (resolved 2026-05-27)
    Repos whose CLAUDE.md is auto-generated by `/sync-ai-instructions`
    (currently `agent-pipeline` with the `ci` stack, and `ai-instructions`
    itself with the `dotnet-blazor` stack template) cannot use Decision #12's
    inline placement — edits get wiped on next regen.

    Pattern: the implementer-role snippet lives in a per-repo
    `WORKFLOW-ROLE.md` at the repo root. `.ai/base-instructions.md`
    contains a conditional reference line that appears in every synced
    CLAUDE.md and tells CC to read `WORKFLOW-ROLE.md` if it exists.
    Consumer repos don't have the file → the reference line is a no-op.

    Result:
    - Hand-written CLAUDE.md repos (e.g. `bridge`) → inline snippet per
      Decision #12. Unchanged.
    - Regenerated CLAUDE.md repos (e.g. `agent-pipeline`, this repo) →
      `WORKFLOW-ROLE.md` referenced from base.

    Accepted cost: two patterns for the same orientation content. Both
    achieve Decision #12's spirit (orientation read early), but via
    different mechanisms. The alternative (standardize on
    `WORKFLOW-ROLE.md` everywhere) was rejected — would force bridge
    to maintain two files when one works fine.

    Implementation:
    - `.ai/base-instructions.md` updated with the conditional reference.
    - `ai-instructions/WORKFLOW-ROLE.md` created.
    - Root `CLAUDE.md` / `SKILL.md` / `.github/copilot-instructions.md`
      will pick up the new reference line on next `/sync-ai-instructions`
      regen (known drift until then).
